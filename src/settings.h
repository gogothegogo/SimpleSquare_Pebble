...
typedef enum {
  ...
  AppKeyBatteryIcon = 4,
  AppKeyDateFormat = 5,
  AppKeyCroatianDate = 6
} AppKey;

static int settings[7]={COLOR_THEME, TIME_SIZE, DATE_SIZE, BLUETOOTH_ALARM, BATTERY_ICON, DATE_FORMAT, CROATIAN_DATE};

static void settings_read () {
  if (persist_exists(AppKeyColorTheme)) {
  ...
  APP_LOG(APP_LOG_LEVEL_INFO, "ColorTheme:%d ", settings[AppKeyColorTheme]);
